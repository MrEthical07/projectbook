...
For browser-based clients, authentication is managed with HTTP-only token cookies.

**Cookie names:** `projectbook_access`, `projectbook_refresh`

**Flow:**
1. Client sends `POST /api/v1/auth/login` with email and password.
2. Backend returns token pair payload (access + refresh token and access expiry metadata).
3. Frontend server auth boundary persists tokens in HTTP-only cookies.
4. Authenticated API calls use the access token.
5. On 401, client triggers `POST /api/v1/auth/refresh` with refresh token and rotates cookie tokens.
6. `POST /api/v1/auth/logout` revokes auth and frontend clears auth cookies.

**Refresh lifetimes:**
| Mode | Duration | Description |
|------|----------|-------------|
| Default | 7 days | Standard refresh lifetime (`remember: false`) |
| Remember me | 30 days | Extended refresh lifetime (`remember: true`) |

Access tokens are intentionally short-lived and refreshed transparently when possible.

### 2.2 JWT Bearer Tokens (Programmatic Client Flow)

For programmatic API clients (mobile apps, CLI tools, third-party integrations), use Bearer tokens directly:

```
Authorization: Bearer <token>
```

The token pair is returned from the login endpoint and should be stored securely by the client.

### 2.3 Auth Lifecycle

| Event | Behavior |
|-------|----------|
| Login | Access/refresh token pair issued |
| Access token expiry | Client attempts refresh flow automatically |
| Refresh failure | Auth cookies are cleared and user must sign in again |
| Logout | Current auth token chain revoked and cookies cleared |
| Password reset | Existing auth sessions/tokens are revoked by backend policy |
| Email verification | No active-session side effects; user signs in after verification |
...
| Access token | Backend-issued short TTL | Authorize API requests |
| Refresh token (default) | 7 days | Renew access token |
| Refresh token (remember) | 30 days | Extended sign-in session |
