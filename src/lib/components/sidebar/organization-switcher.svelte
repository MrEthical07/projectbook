<script lang="ts">
	...
	let selectedOrganizationName = $state<string | null>(null);
	let activeOrganization = $derived.by(
		() =>
			organizations.find((organization) => organization.name === selectedOrganizationName) ??
			organizations[0] ??
			null
	);
</script>
...
<Sidebar.Menu>
	<Sidebar.MenuItem>
		{#if activeOrganization}
			{@const activeSelection = activeOrganization}
			<DropdownMenu.Root>
				<Sidebar.Group>
							...
							{#snippet child({ props })}
								...
								>
									...
									<div class="grid flex-1 text-start text-sm leading-tight">
										...
										<span class="truncate text-xs">{activeSelection.plan}</span>
									</div>
