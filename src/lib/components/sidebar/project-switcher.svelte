<script lang="ts">
	...
	let activeProjectMissing = $derived(currentProjectId.trim().length > 0 && activeProject === null);
...
<Sidebar.Menu>
	<Sidebar.MenuItem>
		<DropdownMenu.Root>
			<Sidebar.Group class="gap-0 space-y-0">
				...
				<Sidebar.GroupContent>
					<DropdownMenu.Trigger>
						{#snippet child({ props })}
							<Sidebar.MenuButton
								{...props}
								size="lg"
								class="mt-2 h-10 w-full border py-0 transition-all duration-200 hover:border-primary data-[state=open]:border-primary"
							>
								<div class="flex aspect-square size-8 items-center justify-center rounded-lg">
									{#if activeProject}
										{@const ActiveProjectIcon = resolveIconComponent(
											activeProject.Icon,
											FolderKanbanIcon
										)}
					...
					<DropdownMenu.Content
						...
						sideOffset={4}
					>
						<DropdownMenu.Label class="text-xs text-muted-foreground">Projects</DropdownMenu.Label>
						...
						{:else}
							{#each projects as project (project.id)}
								...
								<DropdownMenu.Item
									onSelect={() => selectProject(project.url)}
