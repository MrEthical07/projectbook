		...
		<div class="mx-auto flex h-16 max-w-7xl items-center justify-between px-5 sm:px-6">
			...
			<nav class="hidden items-center gap-7 md:flex">
              ...
              {active
								...
								: 'w-0 bg-black/25 group-hover:w-full'}"
						>
						</span>
					</a>
				{/each}
			...
			<div class="flex items-center gap-3">
				...
				<button
					...
					class="flex h-9 w-9 items-center justify-center rounded-lg text-gray-700
                 transition-colors hover:bg-black/8 md:hidden"
		...
		>
			<div class="space-y-1 px-5 py-4">
				{#each links as link}
                   ...
                   {active
							? 'border border-violet-500/20 bg-violet-500/10 font-medium text-gray-700'
							: 'border border-transparent text-gray-600 hover:bg-gray-100 hover:text-gray-800'}"
					>
						{link.label}
