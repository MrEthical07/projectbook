...
const loadForms = async (url: URL) => {
	...
	const resendForm = await superValidate({ email: emailFromURL(url) }, zod4(forgotPasswordSchema), {
...
export const actions: Actions = {
	...
	resetToken: async (event) => {
		...
		const tokenForm = await superValidate(request, zod4(resetPasswordSchema), {
			id: RESET_TOKEN_FORM_ID
		});
		const resendForm = await superValidate(
			{ email: emailFromURL(url) },
			zod4(forgotPasswordSchema),
		...
		try {
			...
			if (isApiRequestError(err)) {
				withFormError(
					tokenForm,
					err.statusCode === 400 ? 'Invalid or expired reset link.' : err.userMessage
				);
				return fail(err.statusCode >= 400 && err.statusCode < 600 ? err.statusCode : 400, {
					otpForm,
					tokenForm,
