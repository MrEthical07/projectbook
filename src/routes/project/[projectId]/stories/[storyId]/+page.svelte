<script lang="ts">
	...
	const isReadOnly = $derived(
		story.status === 'locked' || story.status === 'archived' || !canEditStory
	);
	...
	const toggleSection = (sectionId: string) => {
		addOnSections = addOnSections.map((section) =>
			section.section_id === sectionId ? { ...section, collapsed: !section.collapsed } : section
		);
	};
...
{#key storyId}
		...
		<div class="flex flex-col gap-4 py-2 md:px-20">
			<div class="mt-2 flex flex-col gap-2 rounded-lg bg-background p-2">
				...
				<Input
					...
					disabled={isReadOnly}
					class="h-fit border-0 bg-transparent px-3 py-2 text-lg! shadow-none outline-0"
					placeholder="Story description"
				></Input>
				<div class="mt-2 flex w-full flex-row items-center justify-between px-2">
					...
					<div class="flex flex-row items-center gap-2">
						...
						<Button
							...
							aria-label="Open story metadata"
						>
							<Info class="h-4 w-4" />
						</Button>
						<Button
						...
						<Button
							variant="outline"
							onclick={() => (statusConfirmOpen = true)}
							disabled={!canChangeStoryStatus}>Change Status</Button
						>
					</div>
				</div>
			</div>
			...
			<div class="flex w-full flex-col gap-4 py-2">
				<div class="personaDetails flex w-full flex-col gap-2 rounded-lg bg-background p-4">
						...
						<div class="flex flex-col gap-3">
							...
							<div class="grid w-full grid-cols-1 gap-6 md:grid-cols-4">
								...
								<div class="flex w-full flex-row gap-1">
									...
									<Input
										bind:value={story.persona.edu}
										disabled={isReadOnly}
										id="personaEdu"
										class="w-full"
										type="text"
										placeholder="Ex. B.Tech in CSE"
									></Input>
								</div>
							</div>
						</div>
				...
				<div class="context flex w-full flex-col gap-2 rounded-lg bg-background p-4">
					<div class="flex w-full flex-row items-center gap-2">
					...
					<div class="flex flex-col gap-6 p-2 py-4">
						<div class="flex flex-col gap-2">
							<Label class="text-muted-foreground" for="userContext"
								>User Context or Environment</Label
							>
							<Textarea
								bind:value={story.context}
								disabled={isReadOnly}
								id="userContext"
				...
				<div class="stdf flex w-full flex-col gap-2 rounded-lg bg-background p-4">
					<div class="flex w-full flex-row items-center gap-2">
						<span class="font-medium text-nowrap">Says / Thinks / Does / Feels</span>
						<Separator></Separator>
					</div>
					<div class="grid grid-cols-1 gap-6 p-2 py-4 sm:grid-cols-2">
						<div class="flex flex-col gap-2">
							<Label class="text-muted-foreground" for="empathySays">Says</Label>
							<Textarea
								bind:value={story.empathyMap.says}
								disabled={isReadOnly}
								id="empathySays"
						...
						<div class="flex flex-col gap-2">
							...
							<Textarea
								bind:value={story.empathyMap.thinks}
								disabled={isReadOnly}
								id="empathyThinks"
								placeholder="What user thinks."
							></Textarea>
						</div>
						<div class="flex flex-col gap-2">
							<Label class="text-muted-foreground" for="empathyDoes">Does</Label>
							<Textarea
								bind:value={story.empathyMap.does}
								disabled={isReadOnly}
								id="empathyDoes"
								placeholder="What user does."
							></Textarea>
						</div>
						<div class="flex flex-col gap-2">
							<Label class="text-muted-foreground" for="empathyFeels">Feels</Label>
							<Textarea
								bind:value={story.empathyMap.feels}
								disabled={isReadOnly}
								id="empathyFeels"
								placeholder="What user feels."
							></Textarea>
						</div>
					</div>
				</div>
				<div class="painPoints flex w-full flex-col gap-2 rounded-lg bg-background p-4">
					<div class="flex w-full flex-row items-center gap-2">
					...
					<div class="flex flex-col gap-2">
						<div class="grid grid-cols-1 gap-2">
							{#each story.painPoints as painPoint, index (index)}
								<div
									class="flex flex-row items-center justify-between rounded-lg border border-destructive/50 bg-destructive/5 px-4 py-2 text-sm text-destructive"
								>
									{painPoint}
									{#if !isReadOnly}
										<X class="h-4 w-4 cursor-pointer" onclick={() => removePainPoint(index)}></X>
									{/if}
								</div>
							{:else}
								<div
									class="bg-orange-500/5 px-4 py-2 border border-orange-500/50 text-orange-500 rounded-lg text-sm flex flex-row justify-between items-center"
								>
				...
				<div class="problemHypothesis flex w-full flex-col gap-2 rounded-lg bg-background p-4">
					<div class="flex w-full flex-row items-center gap-2">
					...
					<div class="flex flex-col gap-2">
						<div class="grid grid-cols-1 gap-2">
							{#each story.hypothesis as point, index (index)}
								<div
									class="flex flex-row items-center justify-between rounded-lg border border-primary/50 bg-primary/5 px-4 py-2 text-sm text-primary"
								>
									{point}
									{#if !isReadOnly}
										<X class="h-4 w-4 cursor-pointer" onclick={() => removeHypothesis(index)}></X>
									{/if}
								</div>
							{:else}
								<div
				...
				{#if addOnSections.length > 0}
					<div class="flex flex-col gap-4">
						{#each addOnSections as section (section.section_id)}
							<div class="flex w-full flex-col gap-2 rounded-lg bg-background p-4">
								<div class="flex w-full flex-row items-center gap-2">
									<Button
										variant="ghost"
										size="icon"
												...
												<div class="flex flex-col gap-2">
													{#each (section.content as { successCriteria: string[] }).successCriteria as item, idx (idx)}
														<div class="flex items-center gap-2">
															<Input
																bind:value={
																	(section.content as { successCriteria: string[] })
																		.successCriteria[idx]
																}
																disabled={isReadOnly}
																oninput={(e) => {
												...
												<div class="flex flex-col gap-2">
													{#each (section.content as { supportingJobs: string[] }).supportingJobs as item, idx (idx)}
														<div class="flex items-center gap-2">
															<Input
																bind:value={
																	(section.content as { supportingJobs: string[] }).supportingJobs[
																		idx
																	]
																}
																disabled={isReadOnly}
																oninput={(e) => {
																	const updated = [
																		...(section.content as { supportingJobs: string[] })
																			.supportingJobs
																	];
																	updated[idx] = (e.currentTarget as HTMLInputElement).value;
																	updateSectionContent(
												...
												<div class="flex flex-col gap-2">
													{#each (section.content as { emotionalJobs: string[] }).emotionalJobs as item, idx (idx)}
														<div class="flex items-center gap-2">
															<Input
																bind:value={
																	(section.content as { emotionalJobs: string[] }).emotionalJobs[
																		idx
																	]
																}
																disabled={isReadOnly}
																oninput={(e) => {
																	const updated = [
																		...(section.content as { emotionalJobs: string[] })
																			.emotionalJobs
																	];
																	updated[idx] = (e.currentTarget as HTMLInputElement).value;
																	updateSectionContent(
														...
														<Input
															...
															oninput={(e) => {
																const updated = [
																	...(
																		section.content as {
																			constraints: { description: string; type: string }[];
																		}
																	).constraints
																];
																updated[idx] = {
													...
													<div class="grid grid-cols-1 gap-2 md:grid-cols-[1fr_180px_auto]">
														<Input
															...
															oninput={(e) => {
																const updated = [
																	...(
																		section.content as { risks: { text: string; impact: string }[] }
																	).risks
																];
																updated[idx] = {
																	...updated[idx],
															...
															<Button
																...
																onclick={() => {
																	const updated = [
																		...(
																			section.content as {
																				risks: { text: string; impact: string }[];
																			}
																		).risks
																	];
																	updated.splice(idx, 1);
														...
														<Input
															...
															oninput={(e) => {
																const updated = [
																	...(
																		section.content as {
																			evidence: { source: string; summary: string; link: string }[];
																		}
																	).evidence
																];
																updated[idx] = {
				...
				{/if}
				<div class="notes flex w-full flex-col gap-2 rounded-lg bg-background p-4">
					<div class="flex w-full flex-row items-center gap-2">
		...
		<Dialog.Content>
			...
			<div class="grid gap-2 text-sm">
				<div class="flex items-center justify-between rounded-md border px-3 py-2">
					<span class="text-muted-foreground">Created by</span><span
						>{metadata.createdBy || 'Unknown'}</span
					>
				</div>
				<div class="flex items-center justify-between rounded-md border px-3 py-2">
					<span class="text-muted-foreground">Created at</span><span
						>{metadata.createdAt || 'Unknown'}</span
					>
				</div>
				<div class="flex items-center justify-between rounded-md border px-3 py-2">
					<span class="text-muted-foreground">Last edited by</span><span
						>{metadata.lastEditedBy || 'Unknown'}</span
					>
				</div>
				<div class="flex items-center justify-between rounded-md border px-3 py-2">
					<span class="text-muted-foreground">Last edited at</span><span
						>{metadata.lastEditedAt || metadata.lastUpdated || 'Unknown'}</span
					>
				</div>
				<div class="flex items-center justify-between rounded-md border px-3 py-2">
					<span class="text-muted-foreground">Status</span><span>{story.status.toUpperCase()}</span>
				</div>
			</div>
			<Dialog.Footer>
	...
	<Dialog.Root bind:open={removeSectionOpen} onOpenChange={(open) => open || (removeTarget = null)}>
		<Dialog.Content>
			...
			<Dialog.Footer>
				...
				<Button variant="destructive" onclick={removeSection}>Remove section</Button>
			</Dialog.Footer>
		</Dialog.Content>
	</Dialog.Root>
	...
	<Dialog.Root
		...
		onOpenChange={(open) => {
			statusConfirmOpen = open;
			if (!open) pendingStatus = null;
		}}
	>
		<Dialog.Content>
			<Dialog.Header>
