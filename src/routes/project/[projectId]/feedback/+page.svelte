<script lang="ts">
	...
	import {
		createFeedback as createFeedbackRemote,
		getFeedback as getFeedbackRemote
	} from '$lib/remote/feedback.remote';
...
<svelte:head>
	<title
		>Feedback • {((data as Record<string, unknown>).project as { name?: string } | undefined)
			?.name ?? 'Project'} • ProjectBook</title
	>
	<meta name="description" content="Capture feedback and learnings from testing and validation." />
					...
					<Dialog.Root bind:open={createOpen}>
						...
						<Dialog.Content>
							<Dialog.Header>
								...
								<Dialog.Description
									>Creates feedback in default outcome and redirects to detail page.</Dialog.Description
								>
							</Dialog.Header>
							...
							<Dialog.Footer>
								...
								<Button
									...
									disabled={!createTitle.trim() || isCreatingFeedback}
								>
		...
		<section class="grid gap-3 rounded-lg bg-background p-4 md:grid-cols-4">
			...
			>
				<div class="mb-1 flex items-center justify-between text-xs text-muted-foreground">
					<span>Total Feedback</span><MessageSquareQuote class="size-4" />
				</div>
				<div class="text-2xl font-semibold">{stats.total}</div>
			</button>
			<button
				class="cursor-pointer rounded-md border p-3 text-left"
				onclick={() => applyStatFilter('Validated')}
			>
				<div class="mb-1 flex items-center justify-between text-xs text-muted-foreground">
					<span>Validated</span><CircleCheckBig class="size-4" />
				</div>
				<div class="text-2xl font-semibold text-emerald-500">{stats.validated}</div>
			</button>
			<button
				class="cursor-pointer rounded-md border p-3 text-left"
				onclick={() => applyStatFilter('Invalidated')}
			>
				<div class="mb-1 flex items-center justify-between text-xs text-muted-foreground">
					<span>Invalidated</span><CircleX class="size-4" />
				</div>
				<div class="text-2xl font-semibold text-slate-500">{stats.invalidated}</div>
			</button>
			<button
				class="cursor-pointer rounded-md border p-3 text-left"
				onclick={() => applyStatFilter('Needs Iteration')}
			>
				<div class="mb-1 flex items-center justify-between text-xs text-muted-foreground">
					<span>Needs Iteration</span><AlertTriangle class="size-4" />
				</div>
				...
				<div class="grid gap-2">
					...
					<Select.Root type="single" bind:value={statusFilter}>
						...
						<Select.Content>
							...
							<Select.Item value="Needs Iteration" label="Needs Iteration"
								>Needs Iteration</Select.Item
							>
						</Select.Content>
					</Select.Root>
				</div>
								...
								<Table.Row>
									<Table.Cell>
										<div class="flex flex-wrap items-center gap-2">
											<a class="font-medium hover:underline" href={`./feedback/${row.id}`}
												>{row.title}</a
											>
											{#if !row.hasTaskLink}
												<Badge.Badge class="border-amber-500/20 bg-amber-500/10 text-amber-500"
													>Warning: No Task Linked</Badge.Badge
												>
											{/if}
											{#if row.isOrphan}
									...
									<Table.Cell class="text-center"
										>{row.linkedArtifacts.length > 0
											? row.linkedArtifacts.join(', ')
											: 'None'}</Table.Cell
									>
									<Table.Cell class="text-center">
