<script lang="ts">
	...
	$effect(() => {
		...
		untrack(() => {
			...
			linkedArtifacts = (
				Array.isArray(d.linkedArtifacts) ? structuredClone(d.linkedArtifacts) : []
			) as LinkedArtifact[];
...
<svelte:head>
	...
	<meta name="description" content="View and analyze this feedback entry and linked artifacts." />
				...
				<div class="flex flex-wrap items-center justify-between gap-3 px-3">
					<div class="flex flex-wrap items-center gap-2">
						<Button
							...
							aria-label="Open feedback metadata"
						>
							<Info class="h-4 w-4" />
						</Button>
					...
					<div class="flex items-center gap-3">
						...
						<Button
							size="sm"
							onclick={triggerSave}
							disabled={!canEditFeedback || savePhase === 'saving' || !isDirty}
						>
					...
					<div class="grid gap-3">
						...
						{#if linkedArtifacts.length === 0}
							<Alert.Root variant="destructive">
								...
								<Alert.Description>At least one artifact must be linked.</Alert.Description>
							</Alert.Root>
						{/if}
					...
					<div class="flex flex-wrap items-center justify-between gap-3">
						...
						<Dialog.Root bind:open={statusDialogOpen}>
							...
							<Dialog.Content>
								...
								<div
									class="mt-3 rounded-lg border border-border px-3 py-2 text-xs text-muted-foreground"
								>
			...
			<div class="grid gap-2 text-sm">
				...
				<div class="flex items-center justify-between rounded-md border px-3 py-2">
					<span class="text-muted-foreground">Created at</span><span
						>{data.metadata?.createdAt ?? data.metadata?.createdDate ?? 'Unknown'}</span
					>
				</div>
				<div class="flex items-center justify-between rounded-md border px-3 py-2">
