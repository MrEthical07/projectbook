...
<svelte:head>
	<title
		>Tasks • {((data as Record<string, unknown>).project as { name?: string } | undefined)?.name ??
			'Project'} • ProjectBook</title
	>
	...
	<div class="flex flex-col gap-4 py-2 md:px-20">
					...
					<Dialog.Root bind:open={createOpen}>
						...
						<Dialog.Content>
							<Dialog.Header>
								...
								<Dialog.Description
									>Creates a planned task and redirects to the task page.</Dialog.Description
								>
							</Dialog.Header>
		...
		<section class="grid gap-3 rounded-lg bg-background p-4 md:grid-cols-5">
			...
			>
				<div class="mb-1 flex items-center justify-between text-xs text-muted-foreground">
					<span>Total Tasks</span><ClipboardList class="size-4" />
				</div>
				<div class="text-2xl font-semibold">{stats.total}</div>
			</button>
			<button
				class="cursor-pointer rounded-md border p-3 text-left"
				onclick={() => applyStatFilter('Planned')}
			>
				<div class="mb-1 flex items-center justify-between text-xs text-muted-foreground">
					<span>Planned</span><CircleDashed class="size-4" />
				</div>
				<div class="text-2xl font-semibold text-blue-500">{stats.planned}</div>
			</button>
			<button
				class="cursor-pointer rounded-md border p-3 text-left"
				onclick={() => applyStatFilter('In Progress')}
			>
				<div class="mb-1 flex items-center justify-between text-xs text-muted-foreground">
					<span>In Progress</span><LoaderCircle class="size-4" />
				</div>
			...
			>
				<div class="mb-1 flex items-center justify-between text-xs text-muted-foreground">
					<span>Completed</span><CircleCheckBig class="size-4" />
				</div>
				<div class="text-2xl font-semibold text-emerald-500">{stats.completed}</div>
			</button>
			<button
				class="cursor-pointer rounded-md border p-3 text-left"
				onclick={() => applyStatFilter('Abandoned')}
			>
				<div class="mb-1 flex items-center justify-between text-xs text-muted-foreground">
					<span>Abandoned</span><ArchiveX class="size-4" />
				</div>
		...
		<section class="rounded-lg bg-background p-4">
			<div class="mb-3 text-sm font-medium">
				{viewMode === 'Table' ? 'Tasks Table' : 'Tasks Kanban'}
			</div>
								...
								<Table.Row>
									<Table.Cell>
										<div class="flex flex-wrap items-center gap-2">
											<a class="font-medium hover:underline" href={`./tasks/${row.id}`}
												>{row.title}</a
											>
											{#if row.ideaRejected}
												<Badge.Badge class="border-amber-500/20 bg-amber-500/10 text-amber-500"
													>Warning: Linked Idea Rejected</Badge.Badge
												>
											{/if}
											{#if row.status === 'Completed' && !row.hasFeedback}
												<Badge.Badge class="border-amber-500/20 bg-amber-500/10 text-amber-500"
													>Warning: Completed With No Feedback</Badge.Badge
												>
											{/if}
											{#if row.isOrphan}
					...
					{#each statusOrder as status (status)}
						...
						>
							<div class="mb-3 flex items-center justify-between">
								...
								<Badge.Badge class={statusClass(status)}
									>{groupedForKanban[status].length}</Badge.Badge
								>
							</div>
							<div class="space-y-2">
								{#each groupedForKanban[status] as row (row.id)}
									...
									>
										<a class="text-sm font-medium hover:underline" href={`./tasks/${row.id}`}
											>{row.title}</a
										>
										...
										<div class="mt-2 flex flex-wrap gap-2">
											...
											{#if row.ideaRejected}
												<Badge.Badge class="border-amber-500/20 bg-amber-500/10 text-amber-500"
													>Warning: Idea Rejected</Badge.Badge
												>
											{/if}
											{#if row.status === 'Completed' && !row.hasFeedback}
												<Badge.Badge class="border-amber-500/20 bg-amber-500/10 text-amber-500"
													>Warning: No Feedback</Badge.Badge
												>
											{/if}
										</div>
									</div>
