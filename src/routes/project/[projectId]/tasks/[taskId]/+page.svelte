<script lang="ts">
	...
	let status = $derived<TaskStatus>(
		data.task ? (String((data.task as Record<string, unknown>).status) as TaskStatus) : 'Planned'
	);
	let tempStatus: TaskStatus | null = $derived(
		data.task ? (String((data.task as Record<string, unknown>).status) as TaskStatus) : 'Planned'
	);
	let title = $state('');
	...
	let selectedIdea = $derived(ideaOptions.find((idea) => idea.id === selectedIdeaId) ?? null);
	...
	let deadlineLabel = $derived(
		deadlineDate ? deadlineFormatter.format(deadlineDate.toDate(getLocalTimeZone())) : 'Select date'
	);
	...
	<div class="flex w-full flex-col gap-2 rounded-lg border bg-background p-2">
		...
		<div class="flex flex-col gap-4 py-2 md:px-20">
			<div class="mt-2 flex flex-col gap-2 rounded-lg bg-background p-2">
				...
				<div class="flex flex-wrap items-start justify-between gap-3 px-3">
					<div class="flex flex-wrap items-start gap-3">
						<div class="flex min-w-55 flex-col gap-2">
							...
							{:else}
								<Select.Root type="multiple" bind:value={assignedToIds} disabled={isReadOnly}>
					...
					<div class="flex flex-wrap items-center gap-3">
						<Button
							...
							aria-label="Open task metadata"
						>
							<Info class="h-4 w-4" />
						</Button>
						<div class="mt-4 w-fit rounded-lg bg-accent px-2 py-1 text-sm font-medium">
							{status.toUpperCase()}
						</div>
						<div
							class="mt-4 flex min-h-6 flex-col items-center justify-center text-xs leading-tight text-muted-foreground"
						>
			...
			<div class="flex w-full flex-col gap-4 py-2">
				<div class="flex w-full flex-col gap-3 rounded-lg bg-background p-4">
					...
					{#if selectedIdea}
						...
						{#if selectedIdea.problem.status === 'Archived'}
							<Alert.Root class="border border-orange-500/20 bg-orange-500/10 text-orange-500">
								...
								<Alert.Description>Upstream context is archived.</Alert.Description>
							</Alert.Root>
						{/if}
						<ol class="flex flex-col gap-3">
							<li class="flex flex-col gap-2 md:flex-row md:gap-4">
								<div class="pt-1 text-xs font-semibold text-muted-foreground uppercase md:w-28">
									Idea
								</div>
								<div class="flex flex-1 flex-col gap-2 rounded-lg border border-border p-3">
									...
									<div class="flex flex-wrap items-center gap-2 text-xs text-muted-foreground">
										...
										{#if selectedIdea.status === 'Rejected'}
											<div class="rounded-lg bg-destructive/5 px-2 py-1 text-xs text-destructive">
												Rejected
											</div>
										{/if}
									</div>
				...
				{#if activeModules.includes('plan')}
					<div class="group flex w-full flex-col gap-2 rounded-lg bg-background p-4">
						<div class="flex w-full flex-row items-center gap-2">
							...
							{#each planItems as item, index (index)}
								...
								>
									...
									<Input
										...
										disabled={isReadOnly}
										oninput={(event) =>
											updatePlanItem(index, (event.currentTarget as HTMLInputElement).value)}
									/>
									<Button
										variant="ghost"
										size="sm"
						...
						<div class="flex flex-col gap-3">
							{#each executionLinks as link, index (index)}
								<div
									class="flex flex-wrap items-center gap-3 rounded-md border border-border/60 px-3 py-2"
								>
									<div class="min-w-13 text-xs font-semibold text-muted-foreground">
										Link {index + 1}
									</div>
									<Input
										class="min-w-50 flex-1"
										...
										disabled={isReadOnly}
										oninput={(event) =>
											updateExecutionLink(index, (event.currentTarget as HTMLInputElement).value)}
									/>
									<Button
										variant="ghost"
										size="sm"
				...
				<div class="flex w-full flex-col gap-2 rounded-lg bg-background p-4">
					...
					<div class="flex flex-wrap items-center justify-between gap-3">
						<div class="w-fit rounded-lg bg-accent px-2 py-1 text-sm font-medium">
							{status.toUpperCase()}
						</div>
						<Dialog.Root bind:open={statusDialogOpen}>
							...
							<Dialog.Content>
								...
								<Dialog.Footer>
									...
									<Button disabled={statusMutationPending} onclick={() => changeStatus(tempStatus)}>
	...
	<Dialog.Root bind:open={metadataOpen}>
		<Dialog.Content>
			...
			<div class="grid gap-2 text-sm">
				...
				<div class="flex items-center justify-between rounded-md border px-3 py-2">
					<span class="text-muted-foreground">Deadline</span><span
						>{normalizedDeadline(deadlineDate) || 'Unknown'}</span
					>
				</div>
				<div class="flex items-center justify-between rounded-md border px-3 py-2">
				...
				<div class="flex items-center justify-between rounded-md border px-3 py-2">
					<span class="text-muted-foreground">Status</span><span>{status}</span>
				</div>
			</div>
			<Dialog.Footer>
