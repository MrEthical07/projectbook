<script lang="ts">
	...
	$effect(() => {
		...
		untrack(() => {
			...
			const resolvedModuleContent = d.moduleContent as
				| Record<OptionalModuleKey, string>
				| undefined;
			...
			customPainPoints = Array.isArray(d.customPainPoints)
				...
				: [];
			activeModules =
				Array.isArray(d.activeModules) && d.activeModules.length > 0
				...
				<div class="flex flex-wrap items-center justify-between gap-3 px-3">
					<div class="flex flex-wrap items-center gap-2">
						<Button
							...
							aria-label="Open problem metadata"
						>
							<Info class="h-4 w-4" />
						</Button>
								...
								<Dialog.Content>
										...
										{:else}
											<Select.Root type="single" bind:value={selectedStoryId}>
												<Select.Trigger id="link-story" class="w-full">
													{selectedStoryId
														? (availableStoryOptions.find((story) => story.id === selectedStoryId)
																?.title ??
									...
									<Dialog.Footer>
										...
										<Button
											class={buttonVariants()}
											onclick={linkStory}
											disabled={!selectedStoryId || linkMutationPending}
										>
								...
								<Dialog.Content>
									...
									<Dialog.Footer>
										...
										<Button
											class={buttonVariants()}
											onclick={linkJourney}
											disabled={!selectedJourneyId || linkMutationPending}
										>
								...
								<div class="flex items-start justify-between gap-3">
									<div class="flex flex-col gap-1">
										...
										<span class="text-xs text-muted-foreground"
											>{source.label ?? sourceTypeLabel(source)}</span
										>
									</div>
				...
				<section class="flex w-full flex-col gap-3 rounded-lg bg-background p-4">
					...
					{#if sourceInsights.personas.length === 0 && !sourceInsights.context && sourceInsights.painPoints.length === 0 && sourceInsights.journeyPainPoints.length === 0}
						<div class="text-sm text-muted-foreground">
							No insights available. Link sources to generate insights.
						</div>
					{:else}
						<div class="grid gap-4 text-sm text-muted-foreground md:grid-cols-2">
							{#if sourceInsights.personas.length > 0}
								<div>
									<div class="text-xs font-semibold text-muted-foreground uppercase">Persona</div>
							...
							{#if sourceInsights.context}
								<div>
									<div class="text-xs font-semibold text-muted-foreground uppercase">Context</div>
								...
								<div>
									...
									<ul class="mt-2 list-disc space-y-1 pl-4 text-sm text-foreground">
										{#each sourceInsights.painPoints as point}
											<li>
												{point.text}
												<span class="text-xs text-muted-foreground">({point.sourceLabel})</span>
											</li>
										{/each}
									</ul>
								</div>
									...
									<ul class="mt-2 list-disc space-y-1 pl-4 text-sm text-foreground">
										{#each sourceInsights.journeyPainPoints as point}
											<li>
												{point.text}
												<span class="text-xs text-muted-foreground"
													>({point.journeyName} / {point.stageName})</span
												>
											</li>
										{/each}
									</ul>
					...
					<div class="grid gap-3">
						...
						{:else}
							{#each sourcePainPoints as painPoint (painPoint.id)}
								<div
									class="flex items-center gap-3 rounded-lg border border-border px-4 py-3 {isNotEditable
										? 'cursor-not-allowed'
										: 'cursor-pointer'}"
								>
									<input
									...
									<label
										class="flex flex-col gap-1 {isNotEditable
											? 'cursor-not-allowed text-muted-foreground'
											: 'cursor-pointer'} w-full"
										for={'source-pain-point-' + painPoint.id}
									>
						...
						<Dialog.Root>
							...
							<Dialog.Content>
									...
									<div class="flex items-center gap-2">
										...
										{:else}
											...
											<span class="text-muted-foreground"
												>Link at least one source, or acknowledge this is an orphan problem</span
											>
										{/if}
									</div>
									<div class="flex items-center gap-2">
										...
										{:else}
											<CircleX class="h-4 w-4 shrink-0 text-red-500" />
											<span class="text-muted-foreground"
												>Select a source pain point or add a custom pain point</span
											>
										{/if}
									</div>
									<div class="flex items-center gap-2">
										...
										{:else}
											<CircleX class="h-4 w-4 shrink-0 text-red-500" />
											<span class="text-muted-foreground"
												>Write a final problem statement before locking</span
											>
										{/if}
									</div>
								</div>
								<Dialog.Footer>
									...
									<Button disabled={!canLock() || statusMutationPending} onclick={confirmLock}>
			...
			<div class="grid gap-2 text-sm">
				<div class="flex items-center justify-between rounded-md border px-3 py-2">
					<span class="text-muted-foreground">Created by</span><span
						>{data.metadata?.createdBy ?? data.metadata?.owner ?? 'Unknown'}</span
					>
				</div>
				<div class="flex items-center justify-between rounded-md border px-3 py-2">
					<span class="text-muted-foreground">Created at</span><span
						>{data.metadata?.createdAt ?? 'Unknown'}</span
					>
				</div>
				<div class="flex items-center justify-between rounded-md border px-3 py-2">
					<span class="text-muted-foreground">Last edited by</span><span
						>{data.metadata?.lastEditedBy ?? data.metadata?.owner ?? 'Unknown'}</span
					>
				</div>
				<div class="flex items-center justify-between rounded-md border px-3 py-2">
					<span class="text-muted-foreground">Last edited at</span><span
						>{data.metadata?.lastEditedAt ?? data.metadata?.lastUpdated ?? 'Unknown'}</span
					>
				</div>
				<div class="flex items-center justify-between rounded-md border px-3 py-2">
	...
	>
		<Dialog.Content>
			...
			<Dialog.Footer>
				...
				<Button
					disabled={!canChangeProblemStatus ||
						(pendingStatus === 'Locked' && !canLock()) ||
