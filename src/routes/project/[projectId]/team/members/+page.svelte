<script lang="ts">
	...
	import { cancelProjectInvite, createProjectInvite } from '$lib/remote/project.remote';
	import { getContext } from 'svelte';
	import { can } from '$lib/utils/permission';
	import { toast } from 'svelte-sonner';

	type MemberRole = 'Owner' | 'Admin' | 'Editor' | 'Member' | 'Viewer' | 'Limited Access';
	...
	const filterMembers = (query: string) => {
		...
		return members.filter((member) => {
			return [member.name, member.email, member.role, member.team, member.location]
				.join(' ')
				.toLowerCase()
...
<svelte:head>
	<title
		>Members • {((data as Record<string, unknown>).project as { name?: string } | undefined)
			?.name ?? 'Project'} • ProjectBook</title
	>
	<meta name="description" content="Manage project members, invites, and assigned access roles." />
	...
	<div class="flex flex-col gap-4">
										...
										<div class="grid gap-2">
											...
											>
												...
												<Select.Content>
													...
													<Select.Item value="Limited Access" label="Limited Access"
														>Limited Access</Select.Item
													>
												</Select.Content>
											</Select.Root>
										</div>
		...
		<Card.Root>
			<Card.Header>
				...
				<Card.Description>Invitations awaiting acceptance or follow-up.</Card.Description>
			</Card.Header>
			<Card.Content>
				<Table.Root>
