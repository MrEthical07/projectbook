<script lang="ts">
	...
	let phaseChartData = $derived.by((): PhaseChartDatum[] => [
		{
			phase: 'Empathize',
			...
			<div class="grid gap-3 md:grid-cols-2 xl:grid-cols-5">
				{#each stats as stat (stat.label)}
					...
					>
						...
						<div class={`text-3xl font-semibold tracking-tight ${toneValueClass[stat.tone]}`}>
							{stat.value}
						</div>
					...
					<div class="space-y-2">
						...
						{#each myFeedback as item (item.id)}
							...
							>
								...
								<Badge variant={item.outcome === 'Needs Iteration' ? 'destructive' : 'secondary'}
									>{item.outcome}</Badge
								>
							</a>
						{/each}
					...
					<Card.Content class="space-y-2">
						{#each alerts as alert (alert.label)}
							<div class={`rounded-lg border p-3 ${alertClass[alert.severity]}`}>
								...
								<a
									href={alert.href}
									class="mt-2 inline-flex items-center text-xs font-medium text-primary"
								>
									View <ArrowUpRight class="ml-1 size-3.5" />
								</a>
							</div>
		...
		<section>
			<Card.Root>
				<Card.Header>
					...
					<Card.Description
						>Calendar events and activity logs are available on dedicated pages.</Card.Description
					>
				</Card.Header>
				<Card.Content class="grid gap-3 sm:grid-cols-2">
					...
					>
						...
						<p class="mt-1 text-xs text-muted-foreground">
							View upcoming events, deadlines, and reminders.
						</p>
					</a>
					<a
					...
					>
						...
						<p class="mt-1 text-xs text-muted-foreground">
							Review project-wide actions and recent updates.
						</p>
					</a>
				</Card.Content>
			</Card.Root>
