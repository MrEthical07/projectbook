<script lang="ts">
	...
	let journeyId = $derived(routeParams.journeyId ?? '');
	...
	const isReadOnly = $derived(
		journey.status === 'locked' || journey.status === 'archived' || !canEditJourney
	);
	...
	const canSelectStatusOption = (nextStatus: JourneyStatus): boolean => {
	...
	const confirmStatusChange = async () => {
		if (!pendingStatus || !permissions || !canChangeJourneyStatus || statusMutationPending) return;
	...
	<div class="flex flex-col gap-2 rounded-lg border bg-background p-2">
		...
		<div class="flex flex-col gap-4 py-2 md:px-20">
			<div class="mt-2 flex flex-col gap-2 rounded-lg bg-background p-2">
				...
				<Input
					type="text"
					bind:value={journey.description}
					disabled={isReadOnly}
				...
				<div class="mt-2 flex w-full flex-row items-center justify-between px-2">
					...
					<div class="flex flex-row items-center gap-2">
						...
						<Button
							...
							aria-label="Open journey metadata"
						>
							<Info class="h-4 w-4" />
						</Button>
						<Button
						...
						<Button
							...
							disabled={!canChangeJourneyStatus}>Change Status</Button
						>
					</div>
				</div>
			</div>
			...
			<div class="flex w-full flex-col gap-4 py-2">
					...
					<div class="flex flex-col gap-6 p-2 py-4">
						<div class="flex flex-row gap-6">
							<div class="flex w-full max-w-sm flex-col gap-2">
								...
								<Input
									bind:value={journey.persona.name}
									disabled={isReadOnly}
									id="personaName"
									type="text"
							...
							<div class="flex w-full max-w-sm flex-col gap-2">
								...
								<Input
									bind:value={journey.persona.role}
									disabled={isReadOnly}
									id="personaRole"
									type="text"
						...
						<div class="flex flex-col gap-3">
							...
							<div class="grid w-full grid-cols-1 gap-6 md:grid-cols-4">
								<div class="flex w-full flex-row gap-1">
									...
									<Input
										bind:value={journey.persona.age}
										disabled={isReadOnly}
										id="personaAge"
										class="w-full"
										type="number"
								...
								<div class="flex w-full flex-row gap-1">
									...
									<Input
										bind:value={journey.persona.job}
										disabled={isReadOnly}
										id="personaJob"
										class="w-full"
										type="text"
								...
								<div class="flex w-full flex-row gap-1">
									...
									<Input
										bind:value={journey.persona.edu}
										disabled={isReadOnly}
										id="personaEdu"
										class="w-full"
										type="text"
						...
						<div class="flex flex-col gap-2">
							...
							<Textarea
								bind:value={journey.persona.bio}
								disabled={isReadOnly}
								id="personaBio"
				...
				<div class="journey flex w-full flex-col gap-2 rounded-lg bg-background p-4">
					<div class="flex w-full flex-row items-center gap-2">
					...
					<div class="flex flex-col gap-4 p-2 py-4">
						{#each journey.stages as stage, index}
							<div
								class="rounded-lg border border-border p-4 transition-all duration-200 hover:bg-muted/50"
							>
								<div class="mb-2 flex flex-row items-center justify-between">
									...
									<div class="flex flex-row gap-4">
										<Select.Root
											type="single"
											name="emotion"
											bind:value={stage.emotion}
											disabled={isReadOnly}
										>
											<Select.Trigger
												size="sm"
												class="w-fit min-w-25 px-2! py-0! text-xs! font-medium text-muted-foreground "
											>
												{stage.emotion}
											</Select.Trigger>
											<Select.Content>
												<Select.Group>
													...
													{#each emotions as emotion (emotion)}
														<Select.Item value={emotion} label={emotion}>
															{emotion}
														</Select.Item>
													{/each}
												</Select.Group>
										...
										{#if !isReadOnly}
											<button
												...
												class="p-1 text-destructive opacity-50 transition-opacity hover:opacity-100"
											>
												<X size={16} />
											</button>
										{/if}
								...
								<div class="grid grid-cols-1 gap-4 md:grid-cols-2">
									<div class="flex flex-col gap-1">
										<div class="flex items-center justify-between">
											...
											<Dialog.Root>
												<Dialog.Trigger
													class="{buttonVariants({ variant: 'ghost', size: 'sm' })} mt-0"
													>Add</Dialog.Trigger
												>
												<Dialog.Content class="sm:max-w-100">
													...
													<div class="grid gap-3">
														...
														<Input
															id="action"
															name="action"
															bind:value={newActionByStage[index]}
													...
													<Dialog.Footer>
														<Dialog.Close class={buttonVariants({ variant: 'outline' })}
															>Cancel</Dialog.Close
														>
														<Button type="submit" onclick={() => addAction(index)}
															>Add Action</Button
														>
													</Dialog.Footer>
												</Dialog.Content>
										...
										<ul class="list-inside list-disc text-sm text-foreground">
											{#each stage.actions as action, i}
												<li class="group flex flex-row items-center justify-between">
													...
													{#if !isReadOnly}
														<button
															class="hidden text-destructive opacity-50 transition-opacity group-hover:flex hover:opacity-100"
									...
									<div class="flex flex-col gap-1">
												...
												<Dialog.Content class="sm:max-w-100">
													...
													<div class="grid gap-3">
														...
														<Input
															id="painpoint"
															name="painpoint"
															bind:value={newPainPointByStage[index]}
													...
													<Dialog.Footer>
														<Dialog.Close class={buttonVariants({ variant: 'outline' })}
															>Cancel</Dialog.Close
														>
														<Button type="submit" onclick={() => addPainPoint(index)}
										...
										<div class="space-y-1">
											{#each stage.painPoints as point, i}
												<div
													class="group flex flex-row items-center justify-between rounded border border-red-500/20 bg-red-500/10 px-2 py-1 text-sm text-red-500"
												>
													{point}
													{#if !isReadOnly}
														<button
											...
											{:else}
												<span class="text-sm text-muted-foreground italic"> None identified </span>
											{/each}
										</div>
									</div>
								</div>
						...
						{:else}
							<div
								class="flex bg-destructive/5 border border-destructive/50 px-4 py-2 text-destructive rounded-lg text-sm flex-row justify-between items-center"
							>
						...
						{#if !isReadOnly}
							{#if isAddingStage}
								<div class="flex items-center gap-2 rounded-lg border border-dashed p-4">
									<Input placeholder="Stage Name" bind:value={newStageName} />
									<Button onclick={addStage}>Add</Button>
									<Button variant="ghost" onclick={() => (isAddingStage = false)}>Cancel</Button>
								</div>
							{:else}
								<Button
									variant="secondary"
	...
	>
		<Dialog.Content>
			<Dialog.Header>
				...
				<Dialog.Description>
					{#if pendingStatus === 'locked'}
						Locking makes this journey read-only. You can only archive it afterwards.
			...
			<Dialog.Footer>
				...
				{#if pendingStatus}
					<Button
						disabled={!canChangeJourneyStatus || statusMutationPending}
