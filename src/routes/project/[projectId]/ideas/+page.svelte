<script lang="ts">
	...
	import {
		...
		getIdeas as getIdeasRemote
	} from '$lib/remote/idea.remote';
	...
	const statusClass = (status: IdeaStatus) => {
		if (status === 'Considered') return 'bg-blue-500/10 text-blue-500 border-blue-500/20';
		if (status === 'Selected') return 'bg-emerald-500/10 text-emerald-500 border-emerald-500/20';
		if (status === 'Rejected') return 'bg-slate-500/10 text-slate-500 border-slate-500/20';
...
<svelte:head>
	<title
		>Ideas • {((data as Record<string, unknown>).project as { name?: string } | undefined)?.name ??
					...
					<Dialog.Root bind:open={createOpen}>
						...
						<Dialog.Content>
							<Dialog.Header>
								...
								<Dialog.Description
									>Creates a considered idea and redirects to detail page.</Dialog.Description
								>
							</Dialog.Header>
		...
		<section class="grid gap-3 rounded-lg bg-background p-4 md:grid-cols-4">
			...
			>
				<div class="mb-1 flex items-center justify-between text-xs text-muted-foreground">
					<span>Total Ideas</span><Lightbulb class="size-4" />
				</div>
				<div class="text-2xl font-semibold">{stats.total}</div>
			</button>
			<button
				class="cursor-pointer rounded-md border p-3 text-left"
				onclick={() => applyStatFilter('Considered')}
			>
				<div class="mb-1 flex items-center justify-between text-xs text-muted-foreground">
					<span>Considered</span><Archive class="size-4" />
				</div>
				<div class="text-2xl font-semibold text-blue-700">{stats.considered}</div>
			</button>
			<button
				class="cursor-pointer rounded-md border p-3 text-left"
				onclick={() => applyStatFilter('Selected')}
			>
				<div class="mb-1 flex items-center justify-between text-xs text-muted-foreground">
					<span>Selected</span><CircleCheckBig class="size-4" />
				</div>
			...
			>
				<div class="mb-1 flex items-center justify-between text-xs text-muted-foreground">
					<span>Rejected</span><CircleX class="size-4" />
				</div>
								...
								<Table.Row>
									<Table.Cell>
										<div class="flex flex-wrap items-center gap-2">
											<a class="font-medium hover:underline" href={`./ideas/${row.id}`}
												>{row.title}</a
											>
