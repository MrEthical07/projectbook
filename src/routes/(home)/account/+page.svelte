<script lang="ts">
	...
	let timeFormat = $state<TimeFormat>(
		required(initialAccount().timeFormat as TimeFormat, 'timeFormat')
	);

	let inAppNotifications = $state(
		required(initialAccount().inAppNotifications, 'inAppNotifications')
	);
	let emailNotifications = $state(
		required(initialAccount().emailNotifications, 'emailNotifications')
	);
		...
		<section class="flex flex-col gap-4 rounded-lg bg-background p-4">
			...
			<div class="grid gap-3 md:grid-cols-2">
				<div class="rounded-md border border-border p-3">
					...
					<Button class="mt-3" size="sm" variant="outline" disabled>Export data</Button>
				</div>
				<div class="rounded-md border border-border p-3 md:col-span-2">
