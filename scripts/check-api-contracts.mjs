...
const checks = [
	...
	},
	{
		...
		mustContain: ['handleProjectScopeChange', '[cache:project-switch-clear]', 'user-project:'],
		mustNotContain: []
	},
	{
	...
	},
	{
		...
		mustContain: ['`json:"items"`', '`json:"next_cursor,omitempty"`'],
		mustNotContain: []
	},
	{
	...
	},
	{
		...
		mustContain: ['`json:"success"`', '`json:"request_id,omitempty"`'],
		mustNotContain: ['requestId']
	},
	{
		filePath: 'src/lib/remote/project.remote.ts',
		mustContain: ['`${basePath}/dashboard`', 'my_work'],
		mustNotContain: ['path: `${basePath}/dashboard/events`', 'path: `${basePath}/activity`']
	},
	{
...
for (const check of checks) {
	...
	for (const requiredSnippet of check.mustContain) {
		if (!content.includes(requiredSnippet)) {
			violations.push(`${check.filePath}: missing required snippet \"${requiredSnippet}\"`);
		}
	}

	for (const forbiddenSnippet of check.mustNotContain) {
		if (content.includes(forbiddenSnippet)) {
			violations.push(`${check.filePath}: forbidden legacy snippet found \"${forbiddenSnippet}\"`);
		}
	}
}
